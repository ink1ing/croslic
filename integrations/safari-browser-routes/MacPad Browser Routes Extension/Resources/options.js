(() => {
  ...
  const DEFAULT_MAPPINGS = Object.values(PRESET_SETS).flatMap((preset) => preset.mappings);
  const I18N = {
    en: {
      title: 'MacPad Browser Routes',
      subtitle: 'MacPad browser routes are ready to use. Type a prefix plus your query; use %s in a custom URL template.',
      restore: 'Restore Defaults',
      export: 'Export',
      importFile: 'Import File',
      currentMappings: 'Current Mappings',
      emptyHint: 'No shortcuts yet. Restore MacPad rules or add your own.',
      ...
      saveTitle: 'Save',
      delete: 'Delete',
      repo: 'MacPad on GitHub',
      thanks: 'MacPad browser shortcuts',
      switchLang: '中文',
      presets: {
        ai: 'Import AI',
      ...
      alerts: {
        ...
        restoreConfirm: 'Restore the built-in MacPad shortcut rules? This will replace your current list.',
    ...
    zh: {
      title: 'MacPad 浏览器快捷路由',
      subtitle: 'MacPad 内置规则已可直接使用。输入“前缀 + 内容”跳转；自定义地址用 %s 作为查询占位符。',
      restore: '恢复默认',
      export: '导出',
      importFile: '导入文件',
      currentMappings: '当前映射',
      emptyHint: '暂无快捷规则，可恢复 MacPad 内置规则或手动添加。',
      addNew: '新增映射',
      prefix: '前缀',
      target: '目标 URL 模板',
      add: '添加',
      ...
      alerts: {
        ...
        exists: '该前缀已存在',
        restoreConfirm: '确认恢复 MacPad 内置快捷规则？这会替换当前列表。',
